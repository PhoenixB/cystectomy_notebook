...
model.bloodlossratio_oak_tcaggr_mage_mbmi_cci_gender_ptumor_pnodes_opyear <-
  read_rds(path = path_wd(
    'empirical',
    '2_pipeline',
    '1_generate_models',
    'out',
    'bloodlossratio_oak_tcaggr_mage_mbmi_cci_gender_ptumor_pnodes_opyear',
    ext = 'rds'
  ))
...
tab_model(
  ...
  pred.labels = c(
    ...
    m_age = "Age (standardized)",
    m_bmi = "Body mass index (standardized)",
...
p_map(model.bloodlossratio_oak_tcaggr_mage_mbmi_cci_gender_ptumor_pnodes_opyear) %>%
as_tibble() %>%
knitr::kable()
```
...
pp_check(model.bloodlossratio_oak_tcaggr_mage_mbmi_cci_gender_ptumor_pnodes_opyear, nsamples = 10)
```
...
helper.bmi_values <-
  round((2 * helper.bmi_zscores * helper.bmi_med_mad$bmi_mad) / 0.6745 + helper.bmi_med_mad$bmi_median, digits = 1)
tibble(
  oak = FALSE,
  tcaggr = FALSE,
  m_age = 0,
  m_bmi = helper.bmi_zscores,
  cci = "0",
  gender = "male",
  p_tumor = "0",
  p_nodes = "0"
) %>%
add_predicted_draws(model.bloodlossratio_oak_tcaggr_mage_mbmi_cci_gender_ptumor_pnodes_opyear, re_formula = NA) %>%
ggplot(aes(x = .prediction, colour = factor(m_bmi))) +
geom_freqpoly(binwidth = 0.05) +
labs(title = str_wrap("Distribution of blood loss ratio depending on standardized body mass index", 100), subtitle = str_wrap("Other variables held at reference level or median", 100), x = "Blood loss ratio", y = "Count", colour = str_wrap("Body mass index", 20)) +
coord_cartesian(xlim = c(0, 1)) +
theme_light() +
scale_colour_viridis_d(labels = helper.bmi_values)
