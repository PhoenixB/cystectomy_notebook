...
# helper.model_formula_reduced <-
#   as.formula(
#     glue(
#       "intraop_transfusion ~ ",
#       paste(names(attr(fit.glmstep_raw$terms, "dataClasses")[-1]), collapse = " + ")
#     )
#   )
...
recipe.glm <-
  ...
  step_rm(-any_of(helper.variable_names_reduced), -all_outcomes()) %>%
  step_corr(all_numeric()) %>%
  # step_dummy(all_nominal(), -all_outcomes()) %>%
  step_zv(all_predictors()) %>%
  # step_nzv(all_predictors()) %>%
...
fit.glm %>%
  pull_workflow_fit() %>%
  pluck("fit") %>%
  emmeans(~ p_tumor, data = tibble.transfusion_train) %>%
  contrast("consec", options = list(type = "response", adjust = "mvt")) %>%
  tidy() %>%
  knitr::kable()
...
rsmetrics.glm <-
  collect_predictions(resamples.glm) %>%
  group_by(id, id2) %>%
...
helper.performance_list <- list(
  "Stepwise Logistic Regression" = mp.glm,
  "Elastic Net Logistic Regression" = mp.glmnet,
  "Random Forest" = mp.rf,
  "Support Vector Machine" = mp.svm #,
