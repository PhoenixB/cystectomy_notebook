...
customTwoClassSummary <- function(data, lev = NULL, model = NULL, positive = NULL, negative=NULL) 
{
  ...
  out <- c(rocAUC, 
           ...
           caret::sensitivity(data[, "pred"], data[, "obs"], positive=positive), 
           caret::specificity(data[, "pred"], data[, "obs"], negative=negative))
...
recipe.bleeding_model <-
  ...
  step_relevel(all_outcomes(), ref_level = "yes") %>%
...
list(
  "Stepwise Logistic Regression" = mp.classif_glmstep,
  "Random Forest" = mp.classif_rf,
  "Elastic Net Logistic Regression" = mp.classif_glmnet,
  "Support Vector Machine" = mp.classif_svmradial #,
  # "eXtreme Gradient Boosting Machine" = mp.classif_xgb
) %>%
map(pluck, "measures") %>%
bind_rows(.id = "Model") %>%
arrange(desc(auc)) %>%
modify_at(vars(recall, precision, f1, accuracy, auc), round, 3) %>%
...
helper.cutpoint_table <-
  ...
  arrange(Dist) %>%
  select(Model = model, Threshold = prob_threshold, Sensitivity, Specificity, `Balanced Accuracy`, `Youdens J` = J, Distance = Dist) %>%
  modify_at(vars(Sensitivity, Specificity, `Balanced Accuracy`, `Youdens J`, Distance), round, 3)

helper.cutpoint_table %>%
  knitr::kable(caption = "Optimal probability threshold by distance to optimal model (sensitivity and specificity = 1")
...
list(
  ...
  labs(title = "Distance to optimal model", subtitle = "(1 - Specificity) + (1 - Sensitivity)", x = "Probability threshold", y = "Distance", colour = "Model")
...
# ) %>%
#   predict(newdata = tibble.oak_bleeding_model_test, type = "prob") %>%
#   map2(
#     list(
#       rep(0.29),
#       rep(0.34),
...
#   map(caret::confusionMatrix, reference = tibble.oak_bleeding_model_test$intraop_transfusion, mode = "everything") %>%
