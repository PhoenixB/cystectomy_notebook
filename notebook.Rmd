...
model.bloodlossratio_oak_tcaggr_bmi_cci_gender_ptumor_pnodes_opyear <-
  read_rds(path = path_wd(
    'empirical',
    '2_pipeline',
    '1_generate_models',
...
table.model2 <- tab_model(model.bloodlossratio_oak_tcaggr_bmi_cci_gender_ptumor_pnodes_opyear, pred.labels = c(oakTRUE = "Oral anticoagulation", tcaggrTRUE = "Thrombocyte aggregation inhibition", bmi = "Body mass index", genderfemale = "Female gender", bsp_mocci = "Charleston comorbidity index", bsp_mop_tumor = "Tumor stadium", bsp_mop_nodes = "Nodal stadium"), dv.labels = c("Blood loss ratio"), show.ci = 0.95, rm.terms = c(sprintf("simo_mocci1.%d.",seq(1:9)), sprintf("simo_mop_tumor1.%d.",seq(1:4)), sprintf("simo_mop_nodes1.%d.",seq(1:3))), use.viewer = FALSE)
cat(table.model2$knitr)

p_map(model.bloodlossratio_oak_tcaggr_bmi_cci_gender_ptumor_pnodes_opyear) %>%
as_tibble() %>%
knitr::kable()
```
...
pp_check(model.bloodlossratio_oak_tcaggr_bmi_cci_gender_ptumor_pnodes_opyear, nsamples = 10)
```
...
add_predicted_draws(model.bloodlossratio_oak_tcaggr_bmi_cci_gender_ptumor_pnodes_opyear, re_formula = NA) %>%
ggplot(aes(x = .prediction, colour = factor(bmi))) +
geom_freqpoly(binwidth = 0.05) +
labs(title = str_wrap("Distribution of blood loss ratio depending on body mass index", 100), subtitle = str_wrap("Other variables held at reference level", 100), x = "Blood loss ratio", y = "Count", colour = "Body mass index") +
