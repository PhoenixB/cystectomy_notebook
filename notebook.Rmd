...
table.transfusion <-
  ...
  print(., nonnormal = c(
    ...
    "op_duration_min",
    "crystalloids_mlkgh"
  ), printToggle = FALSE, noSpaces = TRUE, smd = TRUE)
...
helper.multi_metric <-
  metric_set(accuracy, sens, spec, roc_auc, my_brier, mn_log_loss)
...
# Data distribution for rms and nomogram
helper.ddist <- datadist(tibble.transfusion)

options(datadist = "helper.ddist")
...
## Formulas
  ...
  - Blood loss ratio: $BL_r = \frac{BL_a}{BV_e}$^[$BL_r$ = Blood loss ratio, $BL_a$ = Absolute blood loss]
...
rms.glm <-
  lrm(
    intraop_transfusion ~ blood_loss_ratio + oak + scored(p_tumor) + age + op_duration_min + gender + norepinephrine + preop_hb + blood_loss_ratio:gender + blood_loss_ratio:preop_hb,
    data = tibble.transfusion,
    scale = TRUE,
    x = TRUE,
    y = TRUE
  )
...
```{r ml_glm_mp,echo=FALSE}

rsmetrics.glm %>%
  select("Metric" = .metric, "N" = n, "Mean" = mean, "Standard Error" = std_err) %>%
  mutate_if(is.numeric, round, 5) %>%
  knitr::kable(caption = "Metrics from repeated 10-fold cross-validation")

testmetrics.glm %>%
  select("Metric" = .metric, "Estimate" = .estimate) %>%
  mutate_if(is.numeric, round, 5) %>%
  knitr::kable(caption = "Metrics from testing dataset")

thresholds.glm %>%
  select("Threshold" = .threshold, "Sensitivity" = sens, "Specificity" = spec) %>%
  mutate_if(is.numeric, round, 3) %>%
  knitr::kable(caption = "Optimal threshold for distance to perfect model")
...
nomogram.rms_glm <-
  nomogram(
    ...
    fun.at = c(.01, .319, .99),
    funlabel = "Probability of transfusion",
    interact = list(preop_hb = seq(100, 160, by = 30)),
    blood_loss_ratio = seq(.1, .4, by = .15),
    age = seq(55, 80, by = 5),
    op_duration_min = seq(100, 900, by = 100),
    maxscale = 16
  )

plot(nomogram.rms_glm, xfrac = .45, ia.space = 1, col.grid = gray(c(0.8, 0.95)))

print(nomogram.rms_glm)
...
if(file_exists(path = path_wd("paper", "notebook", "notebook_files", "models", "tune.glmnet", ext = "rds"))) {
  ...
  helper.glmnet_grid <-
    grid_max_entropy(
      parameters(model.glmnet),
      size = 50
    ) %>%
    write_rds(file = path_wd("paper", "notebook", "notebook_files", "data", "helper.glmnet_grid", ext = "rds"))

  tune.glmnet <-
    workflow.glmnet %>%
    tune_grid(
      resamples = helper.training_folds,
      grid = helper.glmnet_grid,
...
```{r ml_glmnet_mp, echo=FALSE}

rsmetrics.glmnet %>%
  mutate_if(is.numeric, round, 3) %>%
  knitr::kable(caption = "Metrics from bootstrapping")
...
thresholds.glmnet %>%
  mutate_if(is.numeric, round, 3) %>%
  knitr::kable(caption = "Optimal threshold")
  ...
  helper.rf_grid <-
    grid_max_entropy(
      update(
        ...
        mtry = finalize(
          mtry(),
          recipe.rf %>% prep(tibble.transfusion_train) %>% bake(tibble.transfusion_train) %>% select(-intraop_transfusion)
        )
      ),
    size = 100
    ) %>%
  write_rds(file = path_wd("paper", "notebook", "notebook_files", "data", "helper.rf_grid", ext = "rds"))
...
if(file_exists(path = path_wd("paper", "notebook", "notebook_files", "models", "tune.svm", ext = "rds"))) {
  ...
  write_rds(file = path_wd("paper", "notebook", "notebook_files", "data", "helper.svm_grid", ext = "rds"))

  tune.svm <-
    workflow.svm %>%
    tune_grid(
      resamples = helper.training_folds,
      grid = helper.svm_grid,
      control = control_grid(save_pred = TRUE, event_level = "second"),
      metrics = helper.multi_metric) %>%
...
# Create model performance object ----
mp.svm <-
  model_performance(explainer.svm, cutoff = pluck(thresholds.svm, ".threshold"))
...
From January 4th, 2000 to June 13th, 2016, all consecutive patients undergoing radical cystectomy and urinary diversion at the Inselspital, Bern University Hospital, were included. Exclusion criteria were ….

## Intraoperative blood loss

We estimated blood loss by surgical suction and visual assessment in milliliters. Blood loss ratio was calculated by absolute blood loss divided by estimated blood volume, as described previously^[Lemmens HJ, Bernstein DP, Brodsky JB. Estimating blood volume in obese and morbidly obese patients. Obes Surg. 2006;16(6):773-776. doi:10.1381/096089206777346673].

## Statistical analysis

Continuous variables are presented as median [IQR] and categorical variables as frequencies. Group comparisons were done using the $\chi^2$ test for categorical and the Kruskal-Wallis test for continuous variables. To identify independent risk factors for intraoperative blood product transfusion, we performed multiple logistic regression analyses. To avoid overfitting and assess model performance, we first partitioned data into training (90%) and hold-out sets (10%). Variable selection was done using a stepwise selection procedure minimizing AIC^[Akaike's Information Criterion] using the MASS package for R^[Version 4.0.3]. We evaluated the following variables as predictors: blood loss ratio, use of oral anticoagulants or platelet inhibitors, preoperative hemoglobin, BMI, age, sex, Charlson comorbidity index (CCI), tumor stage, positive nodal stage, duration of surgery, presence of previous surgery, use of noradrenaline, intraoperative crystalloids volume and presence of neoadjuvant chemotherapy and all two-way interactions between blood loss ratio, preoperative hemoglobin, age and gender. All continuous variables were normalized. Multicollinearity between predictors was checked using the variance inflation factor. Marginal means were calculated for ordinal categorical predictors using the emmeans package. Model fit was assessed with the le Cessie-van Houwelingen-Copas-Hosmer global goodness of fit test^[Hosmer DW, Hosmer T, Le Cessie S, Lemeshow S. A comparison of goodness-of-fit tests for the logistic regression model. Stat Med. 1997;16(9):965-980. doi:10.1002/(sici)1097-0258(19970515)16:9<965::aid-sim509>3.0.co;2-o], the Hosmer-Lemeshow test, binned residual plots (performance package) and GiViTI calibration plots (givitiR package)^[Finazzi S, Poole D, Luciani D, Cogo PE, Bertolini G. Calibration belt for quality-of-care assessment based on dichotomous outcomes. PLoS One. 2011;6(2):e16110. Published 2011 Feb 23. doi:10.1371/journal.pone.0016110].

# Results



<!-- For model validation we held out 10% of the data. We repeatedly split the remaining 90% into ten folds for cross-validation for ten repetitions overall.  as well as using repeated 10-fold cross-validation for averaging ROC-AUC were applied to -->


<!-- identify independent risk factors for intraoperative blood product transfusion and reported as Wald $\chi^2$ *P* value and adjusted odds ratio (OR) with 95% confidence intervals (CI). -->